...
function hoaCoverLetter(d: MasterReportData): string {
  ...
  return `<div style="font-family:'Cormorant Garamond',Georgia,serif;color:#1a2744;font-size:18px;line-height:1.58"><p style="margin-bottom:15px"><strong style="font-family:'Plus Jakarta Sans',sans-serif;font-size:13px">Dear ${escapeHtml(contact)},</strong> thank you for the opportunity to introduce Camelot Property Management Services Corp. to the Hills of Monroe Condominium. We understand the community is navigating the operational pressure that follows a winter-related loss event, including insurance coordination, restoration oversight, vendor accountability, board communication, and the need for a more organized operating cadence.</p><p style="margin-bottom:14px">Camelot is ready to support the board with the core management functions we know are needed: financial organization, monthly reporting, budgeting, board support, vendor tracking, insurance-file coordination, resident communication systems, and a modern portal workflow through tools such as Concierge Plus and MDS.</p><p style="margin-bottom:22px">For the on-site project management and facilities layer, we recommend scoping that as a separate local field-operations component after reviewing the rent roll, current budget, recent financials, claim files, vendor contracts, and the board's expected inspection cadence. That keeps the proposal practical: Camelot can lead the executive management platform while pricing local site visits, contractor follow-up, meeting attendance, and facilities coordination around the actual workload.</p><div style="font-family:'Plus Jakarta Sans',sans-serif;font-size:12px;color:#1a2744;line-height:1.7"><strong>Sincerely yours,</strong><br><br>David A. Goldoff<br>Founder & President<br>Camelot Property Management Services Corp.</div></div>`;
}
...
function generateHoaBoardMeetingDeck(d: MasterReportData): string {
  const heroImage = hoaSubjectHeroImage(d);
  const slides = `
<div class="slide slide-dark"><div style="position:absolute;inset:0"><img src="${heroImage}" alt="${escapeHtml(d.buildingName || d.address)} subject community exterior" style="width:100%;height:100%;object-fit:cover;opacity:.38" onerror="this.src='${HOA_ARCHITECTURE_IMAGE}'"></div><div style="position:absolute;inset:0;background:linear-gradient(105deg,rgba(10,16,24,.98),rgba(34,47,58,.86),rgba(34,47,58,.38))"></div><div class="pad" style="position:relative;z-index:3">${logoBadge()}<div style="height:100%;display:flex;flex-direction:column;justify-content:center;max-width:820px"><div style="font-size:13px;color:#B8973A;text-transform:uppercase;letter-spacing:2.5px;font-weight:800">Camelot Property Management Services Corp.</div><h1 style="font-family:'Cormorant Garamond',Georgia,serif;font-size:66px;line-height:.96;color:#F4D26A;font-style:italic;margin:12px 0">Hills of Monroe Condominium</h1><p style="font-size:22px;color:rgba(255,255,255,.86);line-height:1.55">Executive Management &amp; Community Operations Proposal</p></div></div></div>
${hoaExecutiveSummarySlide()}
